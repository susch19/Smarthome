...
class XiaomiTempSensor extends Device<TempSensorModel> {
  XiaomiTempSensor(super.id, super.typeName, {final IconData? icon})
      : super(iconData: icon);
                  ...
                  Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                    Consumer(
                      builder: (final context, final ref, final child) {
                        return Text(
                            (ref
                                .watch(TempSensorModel.temperatureProvider(id))
                                .toStringAsFixed(1)),
                  ...
                  Wrap(
                    ...
                    children: [
                      Consumer(
                        builder: (final context, final ref, final child) {
                          return Text(
                              ("${ref.watch(TempSensorModel.humidityProvider(id)).toStringAsFixed(0)} %"),
                              style: const TextStyle());
                        },
                      ),
                      Container(
                      ...
                      Consumer(
                        builder: (final context, final ref, final child) {
                          return Text(
                              ("${ref.watch(TempSensorModel.pressureProvider(id)).toStringAsFixed(0)} hPa"),
                              style: const TextStyle());
                        },
                      ),
                    ],
                            ...
                            children: [
                              Consumer(
                                builder:
                                  ...
                                  return Text(ref
                                      .watch(
                                          ZigbeeModel.lastReceivedProvider(id))
                                      .toDate());
                                },
                              )
                            ]),
...
class _XiaomiTempSensorScreenState extends ConsumerState<XiaomiTempSensorScreen>
  ...
  void initState() {
    ...
    ref
        .read(apiProvider)
        .appHistoryGet(id: widget.device.id, dt: currentShownTime)
        .then((x) {
      histories.clear();
      for (final hist in x.bodyOrThrow) {
        histories.add(hist);
      }
      setState(() {});
    });
  ...
  void changeDelay(final int delay) {
    widget.device.sendToServer(MessageType.options, Command2.delay,
        ["delay=$delay"], ref.read(apiProvider));
  }
        ...
        return ListView(
          ...
          children: (DeviceManager.showDebugInformation
              ...
              <Widget>[
                ...
                ListTile(
                  title: Text(
                      "Temperature: ${model.temperature.toStringAsFixed(2)} °C"),
                ),
                ListTile(
                  title: Text(
                      "Luftdruck: ${model.pressure.toStringAsFixed(1)} kPa"),
                ),
                ListTile(
                  title: Text(
                      "Luftfeuchtigkeit: ${model.humidity.toStringAsFixed(2)} %"),
                ),
                ListTile(
  ...
  Widget buildGraphViewHumidity() {
    final h =
        histories.firstWhereOrNull((final x) => x.propertyName == "humidity");
    if (h != null) {
  ...
  Widget buildGraphViewTemp() {
    final h = histories
        .firstWhereOrNull((final x) => x.propertyName == "temperature");
    if (h != null) {
      return buildTimeSeriesRangeAnnotationChart(
          h,
          " °C",
  ...
  Widget buildGraphViewPressure() {
    final h =
        histories.firstWhereOrNull((final x) => x.propertyName == "pressure");
    if (h != null) {
      return buildTimeSeriesRangeAnnotationChart(
          h,
          " hPA",
  ...
  Widget buildTimeSeriesRangeAnnotationChart(final History h, final String unit,
      final String valueName, final Color lineColor) {
              ...
              LineSeries<TimeSeriesValue, DateTime>(
                  ...
                  dataSource: historyRecords
                      .map((final x) => TimeSeriesValue(
                          DateTime.utc(1)
                              .add(Duration(milliseconds: x.ts))
                              .toLocal(),
                          x.val,
                          lineColor))
                      .toList(),
                  xValueMapper: (final TimeSeriesValue value, final _) =>
                      value.time,
  ...
  getNewData(final DateTime dt) {
    ...
    ref.read(apiProvider).appHistoryGet(id: widget.device.id, dt: dt).then((x) {
      currentShownTime = dt;
      histories.clear();
      for (final hist in x.bodyOrThrow) {
        histories.add(hist);
