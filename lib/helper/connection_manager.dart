...
class ConnectionManager extends _$ConnectionManager {
  ...
  HubConnectionContainer createHubConnection(String serverUrl) {
  ...
  Future _updateState(final List<Object?>? arguments) async {
    ...
    for (final a in arguments!) {
      ...
      for (var i = 0; i < oldState.length; i++) {
        ...
        final types = typesUntypes.map((x) => x.toString()).toList();
        final hasChanged = ref
            .read(stateServiceProvider.notifier)
            .updateAndGetStores(oldModel.id, updateMap as Map<String, dynamic>);

        final newModel = oldModel.mergeWith(
            BaseModel.fromJson(updateMap as Map<String, dynamic>, types));
