...
class UpdateManager extends StateNotifier<VersionAndUrl?> {
  ...
  static final RepositorySlug repositorySlug =
      RepositorySlug("susch19", "SmartHome");
  ...
  static const String updateNotificationTitle =
      "Neue Version verfügbar"; // Update available
  ...
  static Future<void> initialize() async {
    ...
    onDidReceiveNotificationResponse.stream.listen((final details) {
      if (details.payload != null &&
          details.payload!.startsWith("UpdateManager")) {
        HelperMethods.openUrl(
            details.payload!.substring("UpdateManager".length));
      }
    });
  }
  ...
  static Future<void> checkForNewestVersion() async {
    print(
        "showUpdateNotification called ${lastChecked?.toString() ?? "lastChecked is null"}");
    ...
    if (lastChecked == null ||
        DateTime.now().difference(lastChecked!).inHours > checkEveryHours) {
      ...
      if (versionAndUrl != null &&
          !versionAndUrl.upToDate &&
          versionAndUrl.url != null) {
        _instance.state = versionAndUrl;
      }
    }
  }

  static Future<void> displayNotificationDialog(
      final BuildContext context, final VersionAndUrl versionAndUrl) async {
    ...
    if (Platform.isAndroid) {
      ...
      showDialog(
          ...
          builder: (final BuildContext c) => simple_dialog.SimpleDialog.create(
              ...
              content:
                  updateNotificationBody + versionAndUrl.version.toString(),
  ...
  static Future<void> _showNotification(
      final VersionAndUrl versionAndUrl) async {
    ...
    await flutterLocalNotificationsPlugin.show(
        ...
        payload: "UpdateManager${versionAndUrl.url}");
  }

  static String getVersionString(final VersionAndUrl? newVersion) {
    var v = _packageInfo.version.toString();
  ...
  static Future<VersionAndUrl?> getVersionAndUrl() async {
    ...
    if (asset != null && latestVersion > Version.parse(_packageInfo.version)) {
